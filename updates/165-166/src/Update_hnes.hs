...
instance Transformable PreviousVersion.CPU NextVersion.CPU where
  transform :: PreviousVersion.CPU -> IO (Maybe NextVersion.CPU)
  transform (PreviousVersion.CPU {..}) = do
    pc' <- transform pc
    sp' <- transform sp
    a' <- transform a
    x' <- transform x
    y' <- transform y
    p' <- transform p
    cpuCycles <- transform cycles
    interrupt' <- transform interrupt
    ram' <- transform ram
    return $
      NextVersion.CPU
        <$> pc'
